<template id='feed-item'>
  <div class="feed-container">
    ...
    <div class="feed-card" v-if="activity.type=='poll'" tabindex="0">
      <div class="poll">
        <div
          ...
          style="margin-bottom: 5px; padding:10px; padding-left:25px;"
        >
        ...
        <div class="beside-avatar-box">
          ...
          <h6 class="poll-question">
            <a :href="'localhost:6543/poll/' + activity.id + '/'">{{activity.question}}</a>
          </h6>
          ...
          <div style="display:flex; flex-direction='row'">
            <p class="votes" @click="seeVoters">{{totalVotes}} votes</p>
            <span class="middot">&middot;</span>
            <p class="votes" style="color:darkgrey;cursor:pointer;">{{activity.timeRemaining}}</p>
          </div>

          <button v-on:click="vote" v-show="!userHasVoted && !seenPollResults" id="vote-btn">
          ...
          <button v-on:click="seeResults" v-show="!userHasVoted && !seenPollResults">
          ...
          <button @click="openBreakDownWindow" v-show="userHasVoted || seenPollResults">
      ...
      <div class="beside-avatar-box">
        ...
        <div>
          ...
          <div
            ...
            v-else-if="activity.opinion"
            tab-index="0"
            @click="openOpinion"
          >
            <p class="author-name" style="font-weight:normal; color:teal;">Opinion</p>
            <p class="author-name" style="font-size:bold;">
              {{activity.opinion.userName}}
              <span
                style="color:lightgray; font-weight:normal;"
              >{{activity.opinion.timeAdded}}</span>
            </p>
    ...
    <div class="feed-card" v-else-if="activity.type=='opinion'" tabindex="0" @click="openOpinion">
      ...
      <div class="avatar" v-else @click.stop>
        <img v-if="!activity.userPic" src="https://www.w3schools.com/howto/img_avatar.png">
        <img v-else :src="activity.userPic">
      </div>

      <div class="beside-avatar-box" @click.stop>
        ...
        <div class="comment" style="white-space:">{{activity.opinion}}</div>
        <div style="display:flex; flex-direction:row;">
          <img
        ...
        <button @click="addComment("Agree")" v-if="!activity.userHasVoted">
          <i class="fa fa-check button-icon" aria-hidden="true"></i>Agree
        </button>
        <button @click="addComment('Disagree')" v-if="!activity.userHasVoted">
          <i class="far fa-thumbs-down button-icon"></i>Disagree
        </button>
        <button v-on:click="share">Share</button>
        <button @click="openBreakDownWindow" v-show="userHasVoted">
          <i class="fas fa-chart-pie button-icon"></i>View Breakdown
        </button>
      </div>
    </div>
    ...
    <div class="feed-card" v-else-if="activity.type=='reply'" tabindex="0">
      ...
      <div class="beside-avatar-box">
        ...
        >
          ...
          <p class="author-name" style="font-size:bold;">
            {{activity.comment.commenter}}
            <span
              style="color:lightgray; font-weight:normal;"
            >{{activity.comment.timeAdded}}</span>
          </p>
...
export default {
  ...
  methods: {
    openComment() {
      window.open(
        siteUrl + "/view_comment/" + this.activity.comment.comment_id,
        "_self"
      );
    },
    openOpinion() {
      //if the opinion we are trying to opinion is embedded in another activity such as the comment
      if (this.activity.opinion) {
        window.open(
          siteUrl + "/opinion/" + this.activity.opinion.id + "/",
          "_self"
        );
        return 0;
      }
      //else if not
      window.open(siteUrl + "/opinion/" + this.activity.id + "/", "_self");
      return 0;
    },
    openPoll() {
    ...
    followUser() {
      var vm = this;
      axios
        .post(siteUrl + "/follow/" + this.activity.userId, {})
        .then(response => {
    ...
    unfollowUser() {
      axios
        .post(siteUrl + "/unfollow/" + this.activity.userId, {})
        .then(response => {
    ...
    openBreakDownWindow() {
      ...
      if (this.activity.type == "opinion") {
        window.open(
          siteUrl + "/opinion/demographic-metrics/" + this.activity.id,
          "_self"
        );
      } else {
        window.open(
          siteUrl + "/poll/demographic-metrics/" + this.activity.id,
          "_self"
        );
      }
    },
