...
export default {
  ...
  methods: {
    ...
    openQuoteLocation() {
      ...
      if (comment.opinion) {
        window.open(siteUrl + "/opinion/" + comment.opinion.id + "/", "_self");
      } else {
        //the comment is from a poll
        window.open(siteUrl + "/poll/" + comment.poll.id + "/", "_self");
      }
    },
  ...
  created() {
    var vm = this;
    axios
      .get(siteUrl + "/get_comment/" + commentId)
